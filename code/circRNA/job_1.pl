...
my $master_file = "/qbrc/home/mzhang/projects/circRNA/docs/CLIP_Seq_data_v7.txt";
my $circ_dir = "/qbrc/home/mzhang/projects/circRNA/data/circRNA/";
my $genome_dir = "/qbrc/home/mzhang/projects/circRNA/data/library/";
my $homer_dir = "/qbrc/home/mzhang/projects/circRNA/code/shiny/www/";
...
while (<FILE>) {
	...
	foreach $i (0 .. $#fastq) {
		...
		if ($adaptor{$sample} !~ /not found/) {
			...
			foreach (@adaptors) {		# trim each adaptor iteratively
				print {$fh{$process}} "perl ~/software/packages/remove_adaptor/remove_adaptor.pl fastq " . $clip_dir . $sample . " " . $_ . " 18 0.2 10 unique\n";
		...
		# detect circRNA
		print {$fh{$process}} "perl /qbrc/projects/iproject/circRNA/code/circRNA/find_circRNA.pl " . $clip_dir . $sample . " ";
	...
	# find circRANs
	print {$fh{$process}} "perl /qbrc/projects/iproject/circRNA/code/circRNA/filter_circRNA.pl 2 0.34 20 " . $clip_dir . "all_circ.txt ";
	map {print {$fh{$process}} $clip_dir . $_ . " "} @fastq;
	print {$fh{$process}} "\n";

	# motif analysis for linear reads
	$bg_circRNA = "/qbrc/projects/iproject/circRNA/data/analysis/motif/" . $ref_genome{$items_clip[5]} . "_bg.fa";
	...
	print {$fh{$process}} "findMotifs.pl " . $clip_dir . "/linear_reads_motif fasta " . $homer_dir . $items_clip[0] . "/motifResults_linear/ ";
	print {$fh{$process}} "-p 6 -noconvert -chopify -len 4,5,6,7,8,9,10 -fastaBg " . $bg_circRNA . "\n";

	# motif analysis for circRNA reads
	print {$fh{$process}} "findMotifs.pl " . $clip_dir . "/all_circ.txt_motif fasta " . $homer_dir . $items_clip[0] . "/motifResults/ ";
	print {$fh{$process}} "-p 6 -noconvert -chopify -len 4,5,6,7,8,9,10 -fastaBg " . $bg_circRNA . "\n\n";
}
